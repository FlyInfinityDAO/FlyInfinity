...
contract Smart_DeFi_NetWork is Context {
    ...
    mapping(uint64 => uint32) internal QF;
    ...
    mapping(uint32 => address) internal VrL;
    mapping(address => bool) internal MaxPoint;
    mapping(address => bool) internal Agreement_;
    address internal Founder;
    address internal JY;
    address internal smart_Bank;
    address internal smart_Gift;
    address internal Founder_Wallet;
    address internal Agent;
    address internal Smart_Import;
    ...
    bool internal Set_Gift;
    bool internal smart_Import;
    bool internal changeSwitch;
    string internal Road_Map;
    string internal Founder_Message;

    constructor() {
        Founder = _msgSender();
        stableCoin = IERC20(0x55d398326f99059fF775485246999027B3197955);
        smart_Bank = 0x07418EC67B00D0B3f7a69f0984BB145FC29c85e0;
        Founder_Wallet = 0x00e21f2B131CD5ba0c2e5594B1a7302A6Aa64152;
        smart_Gift = 0xdd4d21f89914fB23d169b982fEab23FeA666f3c8;
        Agent = 0xdd4d21f89914fB23d169b982fEab23FeA666f3c8;
    ...
    function DC(address Up) private {
        ...
        require(Owner_Is_My_Line(Founder_Wallet, Up) == true, "UpLines Are Not Imported");
    ...
    function _Change_Wallet(address X) external {
        ...
        require(
            ((KW[_msgSender()].PO == address(0)) || (DX(KW[_msgSender()].PO) && (KW[_msgSender()].QO == address(0)))
        ...
        VV[A.id] = X;
        Node memory B = KW[A.PO];
        B.UP = X;
        KW[A.PO] = B;
        Node memory C = KW[A.QO];
        C.UP = X;
        KW[A.QO] = C;
        Node memory U = KW[A.UP];
        if (A.YY == false) {
            U.PO = X;
        } else {
            U.QO = X;
    ...
    function _Emergency_Vote() external {
        ...
        voteTotal += Owner_All_Point(_msgSender());
        VrL[VCr] = _msgSender();
        VCr++;
    }
    ...
    function DX(address A) private view returns (bool) {
        return (KW[A].id != 0);
    }

    function ZE(address A) private view returns (bool) {
        if (ZH(A) > 0) {
            for (uint24 i = 0; i < DJ; i++) {
                if (JL[i] == A) {
                    return false;
    ...
    function ZH(address A) private view returns (uint32) {
        uint32 min = KW[A].LT <= KW[A].RT ? KW[A].LT : KW[A].RT;
        if (MaxPoint[A] == false) {
            if (min > 5) {
                min = 5;
            }
    ...
    function IRT(address A) private view returns (bool) {
    ...
    function I_C(address R) private view returns (bool) {
        uint256 size;
        assembly {
            size := extcodesize(R)
        }
    ...
    function ZK() private view returns (uint256) {
        return (stableCoin.balanceOf(address(this)) - (DZ * 5 * 10 ** 18)) / ZI();
    }
    ...
    function Last_Value_Points_Average(uint256 X) external view returns (uint256) {
        require(X <= RCr, " Out Of Range ");
        uint256 Lv = 0;
        for (uint256 i = RCr; i > (RCr - X); i--) {
            Lv += (VPL[i] / 10 ** 18);
        }
        return (Lv / X);
    }
    ...
    function Owner_Big_Side(address R) public view returns (uint32) {
        return KW[R].AL >= KW[R].AR ? KW[R].AL : KW[R].AR;
    }
    ...
    function Owner_Info_Global(address R) public view returns (Node memory) {
        return KW[R];
    }
    ...
    function Owner_Directs(address R) public view returns (address, address) {
        return (KW[R].PO, KW[R].QO);
    }

    function Owner_Left_Right_All(address R) public view returns (uint32, uint32) {
        return (KW[R].AL, KW[R].AR);
    }

    function Owner_Left_Right_Save(address R) public view returns (uint32, uint32) {
        return (KW[R].LT, KW[R].RT);
    }
    ...
    function Owner_UpLines_All_Address(address R) public view returns (address[] memory) {
        address[] memory OUAL = new address[](JK);
        uint32 OUAC;
        address _D_UpLine = KW[R].UP;
        address _D = R;
        while (_D != address(0)) {
            OUAL[OUAC] = _D_UpLine;
            OUAC++;
    ...
    function Smart_Road_Map_() public view returns (string memory) {
        return Road_Map;
    }
    ...
    function _Set_Smart_DeFi_Gift(address X) external {
        require(_msgSender() == Founder, "Just Founder");
        require(Set_Gift == false, "Just 1 Time");
        smart_Gift = X;
        Set_Gift = true;
    }
    ...
    function _Set_Import_Setup_Irreturnable(address R) external {
        require(_msgSender() == Founder, "Just Founder");
        require(smart_Import == false, "Just 1 Times");
        Smart_Import = R;
        smart_Import = true;
    }
