...
contract Smart_DeFi_NetWork is Context {
    ...
    constructor(
        ...
        smart_Bank = _smart_Bank;
        Founder_Wallet = _Founder_Wallet;
        smart_Gift = _smart_Gift;
        Agent = _Agent;
    ...
    function DC(address Up) private {
        ...
        // require(
        //     Owner_Is_My_Line(Founder_Wallet, Up) == true,
        //     "UpLines Are Not Imported"
        // );
    ...
    function DH() private {
        ...
        for (uint24 i = 0; i < DJ; i++) {
            ...
            if (Owner_All_Point(JL[i]) < 100) {
                ...
                if (
                    ((UT * ZO * 9) / 10) > stableCoin.balanceOf(address(this))
                ) {
                    stableCoin.safeTransfer(
                        JL[i],
                        stableCoin.balanceOf(address(this))
    ...
    function _Change_Wallet(address X) external {
        ...
        require(
            ((KW[_msgSender()].PO == address(0)) ||
                (DX(KW[_msgSender()].PO) &&
                    (KW[_msgSender()].QO == address(0))) ||
    ...
    function DX(address A) public view returns (bool) {
        return (KW[A].id != 0);
    }
    ...
    function ZK() private view returns (uint256) {
        return
            (stableCoin.balanceOf(address(this)) - (DZ * 5 * 10 ** 18)) / ZI();
    }
    ...
    function Last_Value_Points_Average(
        uint256 X
    ) external view returns (uint256) {
    ...
    function Owner_Left_Right_All(
        address R
    ) public view returns (uint32, uint32) {
        return (KW[R].AL, KW[R].AR);
    }
    function Owner_Left_Right_Save(
        address R
    ) public view returns (uint32, uint32) {
    ...
    function DC2(address Left_100, address Right_100) private {
        ...
        require(
            Owner_Is_My_Line(KW[_msgSender()].PO, Left_100) == true,
            "Left_100 is not your line"
        );
        require(
            Owner_Is_My_Line(KW[_msgSender()].QO, Right_100) == true,
            "Right_100 is not your line"
        );
        require(Owner_All_Point(Left_100) >= 100, "Left_100 is not +100 point");
        require(
            Owner_All_Point(Right_100) >= 100,
            "Right_100 is not +100 point"
        );
    ...
    function _UnLess_Reward() external {
        require(Owner_All_Point(_msgSender()) > 1000, "Just +1000");
        require(
            block.timestamp > time + 4 hours,
            "UnLess_Reward Time Has Not Come"
        );
        newMember = 0;
        LZ = 0;
        DJ = 0;
